...
class CBitrixBasketComponent extends CBitrixComponent
{
	...
	public function recalculateBasket($arPost)
	{
		...
		while ($arItem = $dbItems->Fetch())
		{
			...
			//print_r($arProduct);
			//exit();
			//$arItem['QUANTITY'] = $arProduct['QUANTITY'] - $arProduct['']
			if (CSaleBasketHelper::isSetItem($arItem))
				continue;
		...
		foreach ($arTmpItems as $arItem)
		{
			...
			if ($arItem["CAN_BUY"] == "Y")
			{
				...
				if (!empty($res))
				    ...
				    if ($res['ERRORS']) {
    				    $arRes["WARNINGS"][key($res['ERRORS'])]= $res["ERRORS"][key($res['ERRORS'])];
				    }
			}
	...
	public function checkQuantity($arBasketItem, &$desiredQuantity)
	{
		...
		$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' =>$arBasketItem["PRODUCT_ID"],'STORE_ID'=>2));
		...
		if (empty($arFieldsTmp) || !isset($arFieldsTmp["QUANTITY"]))
		{
			...
			$arResult["ERRORS"][$arBasketItem["PRODUCT_ID"]] = 'Недостаточно товара на складе';
		}
		else if ($desiredQuantity > doubleval($arProduct["QUANTITY"]-$arProduct['QUANTITY_RESERVED']))
		{
			$arResult["ERROR"] = GetMessage("SBB_PRODUCT_NOT_ENOUGH_QUANTITY", array("#PRODUCT#" => $arBasketItem["NAME"], "#NUMBER#" => $desiredQuantity));
			if ($arProduct["QUANTITY"]-$arProduct['QUANTITY_RESERVED'] <=0) {
			    $arResult["ERRORS"][$arBasketItem["PRODUCT_ID"]] ='Недостаточно товара на складе ';
			} else {
    			$arResult["ERRORS"][$arBasketItem["PRODUCT_ID"]] ='Нельзя заказать более '.($arProduct["QUANTITY"]-$arProduct['QUANTITY_RESERVED']);
			    
			}
			$desiredQuantity = $arProduct["QUANTITY"]-$arProduct['QUANTITY_RESERVED'];
		}
