...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["backButton"]) > 0 && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_bitrix_sessid()))
{
  ...
  // RUFUS
	/* if (in_array($arResult['POST']['~ORDER_PROP_2'],array(2310,2312))) {
		...
		elseif ($arResult["POST"]["do_register"] == "Y" && $arResult["AUTH"]["new_user_registration"] == "Y")
		{
            ...
            *//*
			if (!$arResult["POST"]["~NEW_EMAIL"]) {
				$arResult["POST"]["~NEW_EMAIL"] = "buyer".time().GetRandomCode(2).'@server.loc';
			}*/
			...
			if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
			{
				...
				else
				{
					if ($USER->IsAuthorized())
					{
						...
						LocalRedirect($arParams["PATH_TO_ORDER"].'?isNew=true');
					}
					else
					{
...
else
{
	...
	if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 6)
	{
		...
		$arOrder = array(
			...
			'BASKET_ITEMS' => $arProductsInBasket
		);
		...
		{
			$_PRICE_ALL += $arOneItem["PRICE"] * $arOneItem["QUANTITY"];

		}
		$arResult['BASKET_ITEMS'] = $arOrder["BASKET_ITEMS"];
		$arResult['ORDER_DISCOUNT_PRICE'] = $_PRICE_ALL;
		$arResult["DISCOUNT_PRICE"] = $arResult["ORDER_PRICE"] - $_PRICE_ALL;
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 2)
		{
			if ($arResult['POST']['new_city']) {
				...
				$arResult["POST"]["ORDER_PROP_2"] = $ID;
				$_POST['ORDER_PROP_2'] = $ID;
				$arResult["POST"]["~ORDER_PROP_2"] = $ID;
			}
			// <***************** AFTER 2 STEP
			...
			if (!$arResult["POST"]["~ORDER_PROP_7"])
				$arResult["POST"]["ORDER_PROP_7"] = $arResult["POST"]["~ORDER_PROP_7"] = implode(" ",array($arResult["POST"]['CONTACT_PERSON']['LASTNAME'],$arResult["POST"]['CONTACT_PERSON']['NAME'],$arResult["POST"]['CONTACT_PERSON']['SECONDNAME']));


			if ((!in_array($arResult['POST']['~ORDER_PROP_2'], array(2310,2312))) && $arResult["CurrentStep"] <=3) {
				if (!trim($arResult["POST"]['CONTACT_PERSON']['LASTNAME'])) {
					$arResult["arERRORS"]['LASTNAME'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Фамилия\".<br />";
				}
				if (!trim($arResult["POST"]['CONTACT_PERSON']['NAME'])) {
					$arResult["arERRORS"]['NAME'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Имя\".<br />";
				}
				if (!trim($arResult["POST"]['CONTACT_PERSON']['SECONDNAME'])) {
					$arResult["arERRORS"]['SECONDNAME'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Отчество\".<br />";
				}
				if (!trim($arResult["POST"]['ORDER_PROP_6'])) {
					$arResult["arERRORS"]['ORDER_PROP_6'] = 1;
					// $arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"E-mail\".<br />";
				}

				if (!trim($arResult["POST"]['ORDER_PROP_5'])) {
					$arResult["arERRORS"]['ORDER_PROP_5'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Улица\".<br />";
				}
				if (!trim($arResult["POST"]['ORDER_PROP_8'])) {
					$arResult["arERRORS"]['ORDER_PROP_8'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Дом\".<br />";
				}
				if (!trim($arResult["POST"]['ORDER_PROP_9'])) {
					$arResult["arERRORS"]['ORDER_PROP_9'] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"Квартира\".<br />";
				}
			...
			while ($arOrderProps = $dbOrderProps->GetNext())
			{
				...
				elseif ($arOrderProps["IS_PROFILE_NAME"]=="Y" || $arOrderProps["IS_PAYER"]=="Y" || $arOrderProps["IS_EMAIL"]=="Y" || $arOrderProps["IS_ZIP"]=="Y")
				{
					...
					{
						$arResult["USER_EMAIL"] = ($curVal != '')?$curVal:$USER->GetEmail();
					}
					/*
					if($arOrderProps["IS_ZIP"]=="Y")
					{
						$arResult["DELIVERY_LOCATION_ZIP"] = $curVal;
						if ((!in_array($arResult['POST']['~ORDER_PROP_2'], array(2310,2312))) && strlen($arResult["DELIVERY_LOCATION_ZIP"])<=0)
							$bErrorField = True;
					}*/
				}
				elseif ($arOrderProps["REQUIED"]=="Y")
				{
					if ($arOrderProps['ID'] == 4) {
						if ((!in_array($arResult['POST']['~ORDER_PROP_2'], array(2310,2312))) && strlen($curVal)<=0)
							$bErrorField = True;
					} else {
				...
				if (in_array($arOrderProps['ID'],array(4,10))) {
					if ((!in_array($arResult['POST']['~ORDER_PROP_2'], array(2310,2312))) && strlen($curVal)<=0)
						$bErrorField = True;
				}
				if ($bErrorField) {
					$arResult["arERRORS"]['ORDER_PROP_'.$arOrderProps['ID']] = 1;
					//$arResult["ERROR_MESSAGE"] .= GetMessage("SALE_EMPTY_FIELD")." \"".$arOrderProps["NAME"]."\".<br />";
				}
			}


			if (strlen($arResult["ERROR_MESSAGE"]) > 0 || $arResult["arERRORS"])
				$arResult["CurrentStep"] = 2;

			// RUFUS
			/* if (in_array($arResult['POST']['~ORDER_PROP_2'],array(2310,2312)) && $arResult["CurrentStep"]==3) {
				if ($arResult["ORDER_PRICE"] <2000) {
					$arResult['POST']['~DELIVERY_ID']=2;
					$arResult['DELIVERY_ID']=2;
				} else {
					$arResult['POST']['~DELIVERY_ID']=3;
					$arResult['DELIVERY_ID']=3;
				}
				//$arResult['PAY_SYSTEM_ID']=1;
				//$arResult['POST']['~PAY_SYSTEM_ID']=1;
				//$_REQUEST['PAY_SYSTEM_ID'] = 1;
				$arResult["CurrentStep"] = 4;
			}
			
			if (!in_array($arResult['POST']['~ORDER_PROP_2'],array(2310,2312)) && $arResult["CurrentStep"]==4) {
			    //$arResult['PAY_SYSTEM_ID']=8;
			    //$arResult['POST']['~PAY_SYSTEM_ID']=8;
			    //$_REQUEST['PAY_SYSTEM_ID'] = 8;
			...
			{
				if ($arVariants['ID'] == $arResult['POST']['~ORDER_PROP_2']) {
					$city = $arVariants['CITY_NAME'];
					$city_to_save = $arVariants['ID'];
					break;
				}
			}
			$puser = new CUser;
			$puser->Update($USER->GetID(),
				array(
					'UF_HOUSE'=>$arResult["POST"]["~ORDER_PROP_8"],
					'UF_FLAT'=>$arResult["POST"]["~ORDER_PROP_9"],
					'UF_PORCH'=>$arResult["POST"]["~ORDER_PROP_15"],
					'UF_FLOOR'=>$arResult["POST"]["~ORDER_PROP_16"],
					'PERSONAL_STREET'=>$arResult["POST"]["~ORDER_PROP_5"],
					'PERSONAL_CITY'=>$city_to_save,
					'PERSONAL_MOBILE'=>$arResult["POST"]["~ORDER_PROP_10"],
					'PERSONAL_ZIP'=>$arResult["POST"]["~ORDER_PROP_4"],
					'SECOND_NAME'=>$arResult["POST"]['CONTACT_PERSON']['SECONDNAME'],
					'LAST_NAME'=>$arResult["POST"]['CONTACT_PERSON']['LASTNAME']
				)
			);
		}
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 3)
		{
			...
			if (is_array($arResult["DELIVERY_ID"]))
			{
				...
				$arDeliv = CSaleDeliveryHandler::GetBySID($arResult["DELIVERY_ID"][0]);
				$arResult['DELIVERY'] = $arDeliv->Fetch();
...
if ($USER->IsAuthorized())
{
	...
	if ($arResult["CurrentStep"] == 3)
	{
      ...
      //echo "<pre>"; var_dump($arDeliveryServicesList); echo "</pre>";
    ...
    //echo "<pre>"; var_dump($arResult["DELIVERY"]); echo "</pre>";
