	    ...
	    <div class="catalog-item"  id="<? echo $strMainID; ?>">
            ...
            <span class="catalog-item-cats">
                ...
                while($group = $groups->Fetch())
                    $ar_new_groups[] =  '<a href="'.$group['LIST_PAGE_URL'].$group['ID'].'/">'.$group['NAME'].'</a>';
                
                echo join(", ",$ar_new_groups);
                ?>
                
            </span>
            <?php if ($arItem['PROPERTIES']['CML2_ATTRIBUTES']['VALUE']) {?>
            <div class="choose-taste j-choose-taste">
                <?php foreach ($arItem['PROPERTIES']['CML2_ATTRIBUTES']['DESCRIPTION'] as $k => $v) {
                    $properties[$v][$k] = array('name'=> $arItem['PROPERTIES']['CML2_ATTRIBUTES']['VALUE'][$k],'id'=>$arItem['PROPERTIES']['CML2_ATTRIBUTES']['PROPERTY_VALUE_ID'][$k]);
                }
                $vkus = $properties['Вкус'];
                $current_vkus = array_pop($vkus);
                if ($vkus || $current_vkus) {
                ?>
                <div data-value="<?php echo $current_vkus['id'];?>" class="choose-taste-link j-choose-taste-link"><span><?php echo $current_vkus['name'];?></span> <em></em></div>
                <div class="choose-taste-list-container">
                    <div class="choose-taste-list j-choose-taste-list">
                        <div class="item" data-value="<?php echo $current_vkus['id'];?>"><?php echo $current_vkus['name'];?></div>
                        <?php foreach ($vkus as $k => $v) {?>
                        <div class="item" data-value="<?php echo $v['id'];?>"><?php echo $v['name'];?></div>
                        <?php }?>
                    </div>
                </div>
                <?php }?>
            ...
            <div class="catalog-item-price">
        		...
        		<input type="hidden" class="bx_col_input" id="<? echo $arItemIDs['QUANTITY']; ?>" name="<? echo $arParams["PRODUCT_QUANTITY_VARIABLE"]; ?>" value="<? echo $arItem['CATALOG_MEASURE_RATIO']; ?>">
        			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="to-basket bx_bt_button bx_medium" href="javascript:void(0);" rel="nofollow">
                    <span>В корзину</span></a>
        			<?
        		}
        		?>
            </div>
            <a class="quantity j-quantity" data-id="1" href="#">Узнать наличие в магазинах</a>
        </div>
