...
else
{
	if ($arResult["CurrentStep"] < 6):?>
		<form method="post" action="<?= htmlspecialcharsbx($arParams["PATH_TO_ORDER"]) ?>" name="order_form" id="ordForm">
			<?=bitrix_sessid_post()?>
	<?endif;?>

	

<?if ($arResult["CurrentStep"] > 2 && $arResult["CurrentStep"] <= 7):?>
		<input type="hidden" name="CONTACT_PERSON[LASTNAME]" value="<?= $arResult['POST']["CONTACT_PERSON"]['LASTNAME'] ?>">
		<input type="hidden" name="CONTACT_PERSON[NAME]" value="<?= $arResult['POST']["CONTACT_PERSON"]['NAME'] ?>">
		<input type="hidden" name="CONTACT_PERSON[SECONDNAME]" value="<?= $arResult['POST']["CONTACT_PERSON"]['SECONDNAME'] ?>">
	<?endif?>	
	<?if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 7):?>
		...
		<input type="hidden" name="IS_NEW" value="<?= $arResult['POST']["IS_NEW"] ?>">
	<?endif?>
...
<table width="100%" border="0">
	...
	<?if ($arResult["CurrentStep"] < 6):?>
	<input type="submit" id='fSubm'  class="hidden-input" />
		</form>
	<?endif;?>
<?
}
