...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']))
{
	...
	$rsStore = CCatalogStore::GetList(
			array(),
			array('ACTIVE'=>'Y','ISSUING_CENTER'=>'Y','SHIPPING_CENTER'=>'Y','ID'=>$_GLOBALS['CURRENT_CITY']['PROPERTIES']['stories']['VALUE'])
	);
	...
	foreach ($arResult['OFFERS'] as $keyOffer => $arOffer)
	{
		$arOffer['ID'] = intval($arOffer['ID']);
	 	...
	 	if ($arOffer['AMOUNT'] >0) {
	 		...
	 		$arProduct = array('ID'=>$arOffer['ID'],'SECTION_ID'=>array($arResult['IBLOCK_SECTION_ID']),'PROPERTY_458_VALUE'=>array($arResult['PROPERTIES']['CML2_PRICEGROUP']['VALUE_ENUM_ID']));
	 		...
	 		$arResult['TOTAL_AMOUNT'] =$arResult['TOTAL_AMOUNT']+ $arOffer['AMOUNT'];
	 		$arResult['OFFERS_BUY_PROPS'][] = $arOffer['PROPERTIES']['CML2_TASTE'];
	 	}
