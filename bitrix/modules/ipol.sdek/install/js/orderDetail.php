...
if(!self::$isLoaded && IsModuleInstalled('ipol.kladr')){ // ибо кладр = хорошо
		...
		if(!IPOLSDEK_oExport.wnd){
			...
			IPOLSDEK_oExport.wnd = new BX.CDialog({
				...
				buttons: [
					...
					'<input type=\"button\" value=\"<?=GetMessage('IPOLSDEK_JSC_SOD_ALLTARIFS')?>\"  '+savButStat+'onclick=\"IPOLSDEK_oExport.allTarifs.show()\"/>', // все тарифы
					...
					'<input type=\"button\" value=\"<?=GetMessage('IPOLSDEK_JS_SOD_PACKS')?>\"  onclick="IPOLSDEK_packs.open(); return false;"/>', // места				// печать штрихкода
