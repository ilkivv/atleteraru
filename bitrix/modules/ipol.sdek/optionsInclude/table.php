...
<script type='text/javascript'>
	var IPOLSDEK_table = {
		...
		getTable: function(params){
			...
			IPOLSDEK_table.settedFltr=IPOLSDEK_table.filter.set();

			for(var i in IPOLSDEK_table.settedFltr)
				params[i]=IPOLSDEK_table.settedFltr[i];
			...
			$.ajax({
				...
				success:function(data){
					if(data['ttl']==0){
						if(IPOLSDEK_table.isEmpty(IPOLSDEK_table.settedFltr))
						...
						$('[onclick="IPOLSDEK_table.nxtPg(-1)"]').css('visibility','visible');
						$('[onclick="IPOLSDEK_table.nxtPg(1)"]').css('visibility','visible');
						if(data.cP==1)
							$('[onclick="IPOLSDEK_table.nxtPg(-1)"]').css('visibility','hidden');
						if(data.cP>=data.mP)
							$('[onclick="IPOLSDEK_table.nxtPg(1)"]').css('visibility','hidden');
		...
		killReq(oid,mode){ // отзыв и удаление
			if(confirm('<?=GetMessage("IPOLSDEK_JSC_SOD_IFKILL")?>'))
				$.post(
					...
					{action:'killReqOD',oid:oid,mode:mode},
					function(data){
						if(data.indexOf('GD:')===0){
							alert(data.substr(3));
							IPOLSDEK_table.getTable();
							if(IPOLSDEK_table.ultraKillWnd)
								IPOLSDEK_table.ultraKillWnd.Close();
						}
						else
							alert(data);
					}
		...
		delReq(oid,mode){ // просто удаление
			if(confirm('<?=GetMessage("IPOLSDEK_JSC_SOD_IFDELETE")?>'))
				$.post(
					"/bitrix/js/<?=$module_id?>/ajax.php",
					{action:'delReqOD',oid:oid,mode:mode},
					function(data){
						alert(data);
						IPOLSDEK_table.getTable();
					}
		...
		ultraKill: function(){
			if(!IPOLSDEK_table.ultraKillWnd){
				IPOLSDEK_table.ultraKillWnd = new BX.CDialog({
					...
					height: '170',
					width: '200',
					buttons: ['<input type="button" value="<?=GetMessage('IPOLSDEK_OTHR_killReq_BUTTON')?>" onclick="IPOLSDEK_table.forceKill($(\'#IPOLSDEK_delDeqOrId\').val(),$(\'#IPOLSDEK_delDeqType\').val())"/>']
				});
			}
			else
				$('#IPOLSDEK_delDeqOrId').val('');
			IPOLSDEK_table.ultraKillWnd.Show();
		},
		...
		sort: function(wat,handle){
			...
			{
				IPOLSDEK_table.clrCls();
				handle.addClass("adm-list-table-cell-sort-up");
				IPOLSDEK_table.getTable({'by':wat,'sort':'ASC'});
			}
			else
			{
				...
				else
				{
					...
					IPOLSDEK_table.getTable({'by':wat,'sort':'DESC'});
				}
			}
		},
		...
		nxtPg: function(cntr){
			...
			{
				IPOLSDEK_table.getTable({"page":page});
	...
	$(document).ready(IPOLSDEK_table.getTable);
</script>
...
<tr><td colspan='2'>
		<table id='IPOLSDEK_flrtTbl'>
		  <tbody>
			...
			<tr class='IPOLSDEK_mrPd'>
				<td><?=GetMessage('IPOLSDEK_JS_SOD_STATUS')?> <a href='#' class='PropHint' onclick='return ipol_popup_virt("pop-statuses", this);'></a></td>
				<td>
			...
			<tr class='IPOLSDEK_mrPd'>
				<td colspan='2'><input class="adm-btn" type="button" value="<?=GetMessage('MAIN_FIND')?>" onclick="IPOLSDEK_table.getTable()">&nbsp;&nbsp;&nbsp;<input class="adm-btn" type="button" value="<?=GetMessage('MAIN_RESET')?>" onclick="IPOLSDEK_table.filter.reset()"></td>
			</tr>
		  </tbody>
		</table>
		<br><br>
		<div id='IPOLSDEK_requests'>
			<table class="adm-list-table mdTbl">
				<thead>
					<tr class="adm-list-table-header">
						...
						<td class="adm-list-table-cell sortTr" style='width:50px;' onclick='IPOLSDEK_table.sort("ID",$(this))'><div class='adm-list-table-cell-inner'>ID</div></td>
						<td class="adm-list-table-cell sortTr" style='width:50px;' onclick='IPOLSDEK_table.sort("MESS_ID",$(this))'><div class='adm-list-table-cell-inner'><?=GetMessage('IPOLSDEK_JS_SOD_MESS_ID')?></div></td>
						<td class="adm-list-table-cell sortTr" style='width:77px;' onclick='IPOLSDEK_table.sort("ORDER_ID",$(this))'><div class='adm-list-table-cell-inner'><?=GetMessage('IPOLSDEK_TABLE_ORDN')?></div></td>
						<td class="adm-list-table-cell sortTr" style='width:77px;' onclick='IPOLSDEK_table.sort("STATUS",$(this))'><div class='adm-list-table-cell-inner'><?=GetMessage('IPOLSDEK_JS_SOD_STATUS')?></div></td>
						<td class="adm-list-table-cell sortTr" style='width:77px;' onclick='IPOLSDEK_table.sort("SDEK_ID",$(this))'><div class='adm-list-table-cell-inner'><?=GetMessage('IPOLSDEK_JS_SOD_SDEK_ID')?></div></td>
						<?if(sdekHelper::isConverted()){?>
						...
						<td class="adm-list-table-cell sortTr" style='width:50px;' onclick='IPOLSDEK_table.sort("UPTIME",$(this))'><div class='adm-list-table-cell-inner'><?=GetMessage('IPOLSDEK_TABLE_UPTIME')?></div></td>
					</tr>
				</thead>
			...
			<div class="adm-navigation">
				<div class="adm-nav-pages-block">
					<span class="adm-nav-page adm-nav-page-prev IPOLSDEK_crsPnt" onclick='IPOLSDEK_table.nxtPg(-1)'></span>
					<span class="adm-nav-page-active adm-nav-page" id='IPOLSDEK_crPg'>1</span>
					<span class="adm-nav-page adm-nav-page-next IPOLSDEK_crsPnt" onclick='IPOLSDEK_table.nxtPg(1)'></span>
				</div>
				...
				<div class="adm-nav-pages-number-block">
					<span class="adm-nav-pages-number">
						...
						<select id='IPOLSDEK_tblPgr' onchange='IPOLSDEK_table.getTable()'>
			...
			<input type='button' style='margin-top:20px' value='<?=GetMessage('IPOLSDEK_OTHR_killReq_BUTTON')?>' onclick='IPOLSDEK_table.ultraKill()'>&nbsp;
