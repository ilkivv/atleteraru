...
class cityExport{
	...
	function cityExport($timeLimit=60,$fname='tmpExport.txt'){
		$this->fname = $fname;
	// может отсутствовать csv-шник, если запущен не откуда надо, можно кинуть warning и попробовать скачать его
		if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/'.sdekOption::$MODULE_ID.'/city.csv')){
			if(!sdekOption::requestCityFile()){
		...
		$this->citArray  = explode("\n",file_get_contents($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/'.sdekOption::$MODULE_ID.'/city.csv'));
		...
		while($country=$countries->Fetch()){
			...
			if(
				...
				$country['NAME'] == GetMessage('IPOLSDEK_SYNCTY_RUSSIA')
			){
		...
		if(!$this->country && $hasCntr)
			$this->error = GetMessage('IPOLSDEK_SYNCTY_ERR_NORUSSIA');

		
		$this->arMP = $this->getBitrixMP();

		unlink($_SERVER['DOCUMENT_ROOT']."/bitrix/js/".sdekOption::$MODULE_ID."/city.csv");
	}
	...
	protected function endExport(){
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/bitrix/js/'.sdekOption::$MODULE_ID.'/errCities.json',json_encode(sdekOption::zajsonit($this->errCity)));
