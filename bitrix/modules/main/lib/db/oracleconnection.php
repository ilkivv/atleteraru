...
class OracleConnection extends Connection
{
	...
	public function getNextId($name = "")
	{
		...
		$sql = "SELECT ".$this->sqlHelper->quote($name).".NEXTVAL FROM DUAL";
	...
	public function renameTable($currentName, $newName)
	{
		...
		if ($this->queryScalar("SELECT 1 FROM user_sequences WHERE sequence_name=upper('".$this->sqlHelper->forSql('sq_'.$aiName)."')"))
		{
			// for fields excpet for ID here should be $newName.'_'.$fieldName, see createTable
			$newAiName = $newName;

			// rename sequence
			$this->query('RENAME '.$this->sqlHelper->quote('sq_'.$aiName).' TO '.$this->sqlHelper->quote('sq_'.$newAiName));

			// recreate trigger
			$this->query('DROP TRIGGER '.$this->sqlHelper->quote($aiName.'_insert'));

			$this->query('CREATE OR REPLACE TRIGGER '.$this->sqlHelper->quote($newAiName.'_insert').'
						BEFORE INSERT
						ON '.$this->sqlHelper->quote($newName).'
						FOR EACH ROW
							BEGIN
							IF :NEW.'.$this->sqlHelper->quote('ID').' IS NULL THEN
								SELECT '.$this->sqlHelper->quote('sq_'.$newAiName).'.NEXTVAL
									INTO :NEW.'.$this->sqlHelper->quote('ID').' FROM dual;
							END IF;
						END;'
			);
		}
	...
	{
		$sql = 'CREATE TABLE '.$this->sqlHelper->quote($tableName).' (';
		$sqlFields = array();
		...
		foreach ($fields as $columnName => $field)
		{
			...
			$sqlFields[] = $this->sqlHelper->quote($columnName)
				. ' ' . $this->sqlHelper->getColumnTypeByField($field)
		...
		if (!empty($primary))
		{
			...
			{
				$primaryColumn = $this->sqlHelper->quote($primaryColumn);
			}
		...
		if (!empty($autoincrement))
		{
			foreach ($autoincrement as $autoincrementColumn)
			{
				...
				$this->query('CREATE SEQUENCE '.$this->sqlHelper->quote('sq_'.$aiName));

				$this->query('CREATE OR REPLACE TRIGGER '.$this->sqlHelper->quote($aiName.'_insert').'
						BEFORE INSERT
						ON '.$this->sqlHelper->quote($tableName).'
						FOR EACH ROW
							BEGIN
							IF :NEW.'.$this->sqlHelper->quote($autoincrementColumn).' IS NULL THEN
								SELECT '.$this->sqlHelper->quote('sq_'.$aiName).'.NEXTVAL
									INTO :NEW.'.$this->sqlHelper->quote($autoincrementColumn).' FROM dual;
							END IF;
						END;'
				);
			}
	...
	{
		$this->query('DROP TABLE '.$this->sqlHelper->quote($tableName).' CASCADE CONSTRAINTS');
		...
		if ($this->queryScalar("SELECT 1 FROM user_sequences WHERE sequence_name=upper('".$this->sqlHelper->forSql('sq_'.$aiName)."')"))
		{
			$this->query('DROP SEQUENCE '.$this->sqlHelper->quote('sq_'.$aiName));
		}
	}
