...
class CIBlockCMLImport
{
	...
	function CheckPriceGroup($xml)
	{
	    $propertyId = $this->PROPERTY_MAP["CML2_PRICEGROUP"];
	    $enumXmlId = $xml['ИД'];
	...
	function ImportElement($arXMLElement, &$counter, $bWF, $arParent)
	{
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if(array_key_exists($this->mess["IBLOCK_XML2_ARTICLE"], $arXMLElement))
			{
				...
				// RUFUS editing
			if(array_key_exists("ЦеноваяГруппа", $arXMLElement)) {
			    $arElement["PROPERTY_VALUES"][$this->PROPERTY_MAP["CML2_PRICEGROUP"]] = array(
			            "n0" => array(
			                    "VALUE" => $this->CheckPriceGroup($arXMLElement["ЦеноваяГруппа"]),
			                    "DESCRIPTION" => false,
			            ),
			    );
			} else {
		...
		elseif($arElement["ID"] && $this->bCatalog && $this->next_step["bOffer"])
		{
			...
			if(isset($arElement["STORE_AMOUNT"])) {
			    ...
			    $this->ImportStoresAmount(array(), $arElement["ID"], $counter);
			}
		}
