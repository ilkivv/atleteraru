...
//							$arUserGroups[] = SONET_RELATIONS_TYPE_FRIENDS2;
	...
	public static  function GetPropertyValuesArray(&$result, $iblockID, $filter)
	{
		...
		$selectFileMultiply = array('PROPERTY_VALUE_ID' => SORT_NUMERIC);
			...
			foreach ($propertiesList as $code => $property)
			{
				...
				if ('Y' === $property['MULTIPLE'])
				{
					...
					else
					{
						...
						if ('F' == $property['PROPERTY_TYPE'] && $existElementPropertyID && !isset($userTypesList[$property['ID']]))
						{
							Collection::sortByColumn($elementValues[$code], $selectFileMultiply);
						}
					}
