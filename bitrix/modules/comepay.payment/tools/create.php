...
if($row['ORDER_ID']){
	...
	while($data = $arResult->Fetch()){
		...
		if(CComepayPayment::CreateBill($data, $_REQUEST['phone'], $_REQUEST['card'], $response)){
	...
	if($row['CREATED']==1||$orders[$bill_id]['CREATED']==1){
		...
		$port = (CComepayPayment::getPort()!='443')?':'.CComepayPayment::getPort():'';
		header('Location: https://'.$server.$port.'/Order/external/main.action?shop='.CSalePaySystemAction::GetParamValue("PRV_ID")
			.'&transaction=BX'.$bill_id.'&successUrl='.urlencode('http://'.$_SERVER['HTTP_HOST'].'/comepay_success.php').'&failUrl='.urlencode('http://'.$_SERVER['HTTP_HOST'].'/comepay_fail.php').'&terminalBackUrl='.urlencode('http://'.$_SERVER['HTTP_HOST'].'/comepay_success.php'));
	}
