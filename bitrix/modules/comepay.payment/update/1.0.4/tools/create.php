...
if($row['ORDER_ID']){
	...
	if($row['CREATED']==1||$orders[$bill_id]['CREATED']==1){
		...
		header('Location: https://'.$server.'/Order/external/main.action?shop='.CSalePaySystemAction::GetParamValue("PRV_ID")
			.'&transaction='.$bill_id.'&successUrl='.urlencode('http://'.$_SERVER['HTTP_HOST'].'/comepay_success.php').'&failUrl='.urlencode('http://'.$_SERVER['HTTP_HOST'].'/comepay_fail.php'));
	}
