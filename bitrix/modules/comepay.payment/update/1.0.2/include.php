...
class CComepayPayment {
	...
	static function getServer(){
		return CSalePaySystemAction::GetParamValue("TEST")?'193.104.57.23':'money.comepay.ru';
	}
	...
	static function Request($url,$data,$headers=array(), $method = 'PUT') {
		...
		$fp = fsockopen("ssl://".$host, 439, $errno, $errstr, 30);
		$result = '';
		if (!$fp) {
    ...
    static function cancelBill($bill_id) {
    	...
    	$result = CComepayPayment::Request('/api/prv/'.CSalePaySystemAction::GetParamValue("PRV_ID").'/bills/'.$bill_id,'='.urlencode('status=rejected'),$headers,'PATCH');
    }
    ...
    static function CreateBill($data, $phone, &$response) {
    	$headers = array(
		'Accept: text/json',
			'Authorization: Basic '.base64_encode(CSalePaySystemAction::GetParamValue("SHOP_LOGIN").':'.CSalePaySystemAction::GetParamValue("SHOP_PASS")),
			'Content-Type: application/x-www-form-urlencoded; charset=utf-8',
    	...
    	$url = '/api/prv/'.CSalePaySystemAction::GetParamValue("PRV_ID").'/bills/'.$data['BILL_ID'];
		$rawdata = array(
			'user'=>'tel:'.$phone,
			'amount'=>$data['SUM'],
			'ccy'=>'RUB',
		...
		if($response && $response->response->result_code==0){
			return true;
		} else {
			return false;
		}
