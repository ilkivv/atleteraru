...
/*						elseif (strncmp($field_name, 'SP_', 3) == 0)
						{
							$mxValue = '';
							$strKey = substr($field_name,3);
							$intSepPos = strrpos($strKey,'_');
							if (false !== $intSepPos)
							{
								$intKey = intval(substr($strKey,$intSepPos+1));
								$strKey = substr($strKey,0,$intSepPos);
								if (0 <= $intKey)
								{
									$strFullKey = '~'.$strKey.$intKey;
									if (isset($arPath[$strFullKey]))
									{
										$mxValue = $arPath[$strFullKey];
									}
								}
							}
							$arTuple[] = $mxValue;
