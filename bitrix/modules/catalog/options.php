...
<tr class="heading">
	...
	{
		?><option value="<? echo htmlspecialcharsbx($lcur_res["CURRENCY"]) ?>"<? echo (isset($arVal[$lcur_res["CURRENCY"]]) ? ' selected' : ''); ?>><? echo htmlspecialcharsex($lcur_res["CURRENCY"].(!empty($lcur_res['FULL_NAME']) ? ' ('.$lcur_res['FULL_NAME'].')' : '')); ?></option><?
	}
	if (isset($lcur_res))
		unset($lcur_res);
	unset($lcur, $order1, $by1, $strVal, $arVal);
	?></select>
		</td>
	</tr>
<?
