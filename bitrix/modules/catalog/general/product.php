...
class CAllCatalogProduct
{
	...
	public function GetMaxPrice($intProductID, $quantity = 1, $arUserGroups = array(), $renewal = "N", $arPrices = array(), $siteID = false, $arDiscountCoupons = false)
	{
	    global $APPLICATION;
	...
	public function GetOptimalPrice($intProductID, $quantity = 1, $arUserGroups = array(), $renewal = "N", $arPrices = array(), $siteID = false, $arDiscountCoupons = false)
	{
		...
		if (!isset($arPrices) || !is_array($arPrices))
			$arPrices = array();
        if ($GLOBALS['CURRENT_CITY']['PROPERTIES']['PRICETYPE']['VALUE']) {
            $_GLOBALS['CURRENT_CITY']['PROPERTIES']['PRICETYPE']['VALUE'] = $GLOBALS['CURRENT_CITY']['PROPERTIES']['PRICETYPE']['VALUE'];
        }
	    if ($_GLOBALS['CURRENT_CITY']['PROPERTIES']['PRICETYPE']['VALUE']) {
	        $res = GetCatalogGroups(
	                'NAME','ASC'
	        );
	        while ($row = $res->Fetch()) {
	        if (trim($row['NAME']) == trim($_GLOBALS['CURRENT_CITY']['PROPERTIES']['PRICETYPE']['VALUE'])) 
	            $arCatGroups = array($row['ID']);
	        }
		...
		if (empty($arPrices))
		{
		    $arFilter = array(
						"PRODUCT_ID" => $intProductID,
						"GROUP_GROUP_ID" => $arUserGroups,
		               // "CATALOG_GROUP_ID" => $arCatGroups,
						"GROUP_BUY" => "Y",
						"+<=QUANTITY_FROM" => $quantity,
						"+>=QUANTITY_TO" => $quantity
					);
		    if ($arCatGroups) $arFilter["CATALOG_GROUP_ID"] = $arCatGroups;
			$arPrices = array();
			$dbPriceList = CPrice::GetListEx(
				array(),
				$arFilter,
		...
		{
			foreach ($arPrices as $k => $arOnePrice)
			{
				$arOnePrice['ELEMENT_IBLOCK_ID'] = $intIBlockID;
				$arPrices[$k] = $arOnePrice;
			    if ($arCatGroups) {
			        if (!in_array($arOnePrice['CATALOG_GROUP_ID'],$arCatGroups)) {
			            unset($arPrices[$k]);
			        }
			    }
