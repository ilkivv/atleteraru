    ...
    foreach ($arID as $ID)
    {
        ...
        switch ($_REQUEST['action'])
        {
            case "delete":
                ...
                $DB->StartTransaction();
                CSaleBasket::DeleteAll($ID, false);
                CSaleUser::Delete($ID);
                $DB->Commit();
...
$lAdmin->AddHeaders(array(
	...
	array("id" => "FUSER_ID", "content" => GetMessage("SB_FUSER_ID"), "sort" => "FUSER_ID", "default" => true),
...
{
	$row =& $lAdmin->AddRow($arBasket["FUSER_ID"], $arBasket);
	...
	if ($saleModulePermissions >= "W")
	{
	    ...
	    $arActions[] = array("ICON"=>"delete", "TEXT"=>'Удалить', "ACTION"=>"if(confirm('".GetMessage('SALE_CONFIRM_DEL_MESSAGE')."')) ".$lAdmin->ActionDoGroup($arBasket["FUSER_ID"], "delete"));
	}
