...
class CSaleOrderLoader
{
	...
	function nodeHandler(CDataXML $value)
	{
		...
		if(!empty($value[GetMessage("CC_BSC1_DOCUMENT")]))
		{
			...
			if(!empty($arOrder))
			{
			    /* RUFUS ADD COUPON TO USER FROM 1C
			    if ($arOrder["DISCOUNT_COUPON"]) {
			        $rsOrder = CSaleOrder::GetList(array('ID' => 'DESC'), array('ID' => $arOrder['ID']));
			        $arr = $rsOrder->Fetch();
			        $user = new CUser;
			        $user->Update($arr['USER_ID'],array('UF_COUPON'=>$arOrder["DISCOUNT_COUPON"]));
	            }
	            */
						...
						if($arOrder["VERSION_1C"] != $orderInfo["VERSION_1C"] || (strlen($orderInfo["VERSION_1C"]) <= 0 || strlen($arOrder["VERSION_1C"]) <= 0)) // skip update if the same version
						{
							...
							if($orderInfo["PAYED"] != "Y" && $orderInfo["ALLOW_DELIVERY"] != "Y" && $orderInfo["STATUS_ID"] != "F")
							{
								...
								while ($arBasket = $dbBasket->Fetch())
								{
									...
									if(!empty($arOrder["items"][$arBasket["PRODUCT_XML_ID"]]))
									{
                                        ...
                                        /** RUFUS **/
										if ($arOrder['DISCOUNT_COUPON']) {
    										$arOrder["items"][$arBasket["PRODUCT_XML_ID"]]["DISCOUNT_COUPON"] = $arOrder['DISCOUNT_COUPON'];
    										$orderInfo['DISCOUNT_COUPON'] = $arOrder['DISCOUNT_COUPON'];
										}
										/** END RUFUS **/
									}
									else
								...
								foreach($arOrder["items"] as $itemID => $arItem)
								{
								    ...
								    print_r($arItem);
								    echo "\n";
								    echo $arItem["TYPE"]." == ".GetMessage("CC_BSC1_ITEM")."\n";*/
									if($arItem["CHECKED"] != "Y")
									{
										if($arItem["TYPE"] == GetMessage("CC_BSC1_ITEM") || $arItem['NAME'] != 'Доставка заказа')
										{
											if($arBasketFields = $this->prepareProduct4Basket($itemID, $arItem, $orderId, $orderInfo))
											{
											   // print_r($arBasketFields);
											   // echo 'prepared';
                                ...
                                ob_clean();
                                $filename = '/home/hosts/redramka/dev/dev2/www/tmp/' . intval(microtime(1));
                                file_put_contents($filename, $contents);*/
								if($bNeedUpdate)
								{
									$arErrors = array();
									CSaleBasket::DoSaveOrderBasket($orderId, $orderInfo["LID"], $orderInfo["USER_ID"], $arShoppingCart, $arErrors,array($orderInfo['DISCOUNT_COUPON']));
								}
							...
							else
							{
								...
								*/
								...
								$date1 = CDatabase::FormatDate(str_replace("T", " ", $arOrder["TRAITS"][GetMessage("CC_BSC1_1C_STATUS_DATE")]), "YYYY-MM-DD HH:MI:SS", CLang::GetDateFormat("FULL", LANG));
								$date2 =  $orderInfo['DATE_STATUS'];
								$date2 = new DateTime($date2,new DateTimeZone('ASIA/NOVOSIBIRSK'));
								$date1 = new DateTime($date1,new DateTimeZone('ASIA/NOVOSIBIRSK'));
								if ($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_ORDER_STATUS_ID")] !=$orderInfo['STATUS_ID']) {
									if ($date1>$date2) {
										CSaleOrder::StatusOrder($orderId, $arOrder["TRAITS"][GetMessage("CC_BSC1_1C_ORDER_STATUS_ID")]);
										$arAditFields["UPDATED_1C"] = "Y";
									}
								}
					...
					if(!empty($arOrder["AGENT"]) && strlen($arOrder["AGENT"]["ID"]) > 0)
					{
						...
						if(IntVal($arOrder["PERSON_TYPE_ID"]) > 0)
						{
							...
							if(IntVal($arOrder["USER_ID"]) > 0)
							{
								...
								if($arOrder["ID"] = CSaleOrder::DoSaveOrder($orderFields, $arAditFields, 0, $arErrors,array($orderInfo['DISCOUNT_COUPON'])))
								{
	...
	function collectOrderInfo($value)
	{
		...
		if($value["#"][GetMessage("CC_BSC1_OPERATION")][0]["#"] == GetMessage("CC_BSC1_ORDER"))
		{
            ...
            if (isset($value['#']['ДисконтнаяКарта'])) {
                $arOrder['DISCOUNT_COUPON'] =$value['#']['ДисконтнаяКарта'][0]['#']['НомерКарты'][0]['#'];
            }
		...
		if(CModule::IncludeModule("iblock"))
		{
			...
			if($arIBlockElement = $dbIBlockElement->Fetch())
			{
				...
				$arFields = array(
					...
					"MODULE" => "catalog",
				    "DISCOUNT_COUPON"=>$orderInfo['DISCOUNT_COUPON'],
