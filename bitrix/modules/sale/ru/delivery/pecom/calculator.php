...
class Calculator
{
	...
	protected function parseCalcResult($jsonInfo)
	{
		...
		if(is_array($arInfo) && !empty($arInfo))
		{
			if(isset($arInfo[$this->profileId][2]))
			{
				...
				if(isset($arInfo["deliver"][2]) && intval($arInfo["deliver"][2]) > 250 || (\CDeliveryPecom::isConfCheckedVal($this->arConfig, 'SERVICE_DELIVERY_ENABLED'))  || $arInfo['ldeliver']) {
    					$price += intval($arInfo["deliver"][2]);
				}

				if (isset($arInfo["alma_auto"][2])) {
				    $price += intval($arInfo["alma_auto"][2]);
				}
	...
	{	
		...
		for($i = $this->packsCount-1; $i >= 0; $i--)
		{
			...
			$width = round(floatval($item["DIMENSIONS"]["WIDTH"]+0.1)/$measureCoeff, 2);
			$lenght = round(floatval($item["DIMENSIONS"]["LENGTH"]+0.1)/$measureCoeff, 2);
			$height = round(floatval($item["DIMENSIONS"]["HEIGHT"]+0.1)/$measureCoeff, 2);
			$volume = 0.1;//floatval($item["VOLUME"]/(pow($measureCoeff,3)));
