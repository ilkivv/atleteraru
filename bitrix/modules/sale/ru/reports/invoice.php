...
?>
	<br>Адрес:
	<?
	echo $arOrderProps["LOCATION"].", " . $arOrderProps["ADDRESS"] . ' ' . $arOrderProps["HOUSE"]. ' кв ' . $arOrderProps["ROOM"] . (($arOrderProps['porch'])?' подъезд: '.$arOrderProps['porch']:''). (($arOrderProps['floor'])?' этаж: '.$arOrderProps['floor']:'');
	...
	elseif(strlen($arOrderProps["F_ADDRESS"])>0)
		echo $arOrderProps["F_ADDRESS"];
*/
?>	<br>Контактное лицо: <b><?echo $arOrderProps["CONTACT_PERSON"];?></b>
	<?
	if (strlen($arOrderProps["PHONE"])>0)
		echo "<br>Телефон: ".$arOrderProps["PHONE"];
...
//if ($db_basket->ExtractFields("b_")):
	...
	<table border="0" cellspacing="0" cellpadding="2" width="100%">
		...
		<?php 
		$n = 1;
		if ($arOrderProps['CARD_REQUERE'] == 'Y') {?>
		<tr >
			...
			<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-right:none; border-top:none;">
				...
				<td align="right" bgcolor="#ffffff" style="border: 1pt solid #000000; border-top:none;">
					<?echo number_format((0)*$arQuantities[$mi], 2, ',', ' ') ?>
				</td>
		</tr>
		<?php 
	}
