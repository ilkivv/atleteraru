...
BX.IM = function(domNode, params)
{
	...
	this.revision = 34; // api revieion - check include.php
	this.errorMessage = '';
	...
	BX.ajax({
		...
		onsuccess: BX.delegate(function(data)
		{
			if (data.ERROR == '')
			{
				...
				if (this.webrtc.popupTransferDialogContactListElements != null)
				{
					this.webrtc.popupTransferDialogContactListElements.innerHTML = '';
					BX.adjust(this.webrtc.popupTransferDialogContactListElements, {children: this.contactListPrepare({'groupOpen': true, 'viewOffline': false, 'viewGroup': false, 'extra': false, 'searchText': this.webrtc.popupTransferDialogContactListSearch.value})});
				}
			}
			else
			{
...
{
	if (!this.enableInVersion(20) || !this.BXIM.init || !this.ready() || !this.webrtc.callActive || this.topmostWindow || this.phoneTransferEnabled)
		return false;
...
BX.IM.WebRTC.prototype.callOverlayProgress = function(status)
{
	...
	else if (status == 'wait' || status == 'offline' || status == 'error')
	{
		if (status == 'offline')
		{
			BX.removeClass(this.callOverlay, 'bx-messenger-call-overlay-online');
...
BX.IM.WebRTC.prototype.openTransferDialog = function(params)
{
	...
	this.popupTransferDialog = new BX.PopupWindow('bx-messenger-popup-transfer', bindElement, {
		...
		content : BX.create("div", { props : { className : "bx-messenger-popup-newchat-wrap" }, children: [
			...
			BX.create("div", { props : { className : "bx-messenger-popup-newchat-box bx-messenger-popup-newchat-dest bx-messenger-popup-newchat-dest-even" }, children: [
				...
				this.popupTransferDialogContactListSearch = BX.create("input", {props : { className : "bx-messenger-input" }, attrs: {type: "text", placeholder: BX.message('IM_M_SEARCH_PLACEHOLDER'), value: ''}})
			]}),
			this.popupTransferDialogContactListElements = BX.create("div", { props : { className : "bx-messenger-popup-newchat-box bx-messenger-popup-newchat-cl" }, children: this.messenger.contactListPrepare({'viewGroup': true, 'viewChat': false, 'viewOffline': false, 'extra': false, 'groupOpen': true, 'searchText': ''})})
		]})
	});
	...
	BX.bind(this.popupTransferDialogContactListSearch, "keyup", BX.delegate(function(event){
		...
		{
			this.popupTransferDialogContactListSearch.value = '';
			var item = BX.findChild(this.popupTransferDialogContactListElements, {className : "bx-messenger-cl-item"}, true);
			if (item)
			{
				if (this.popupTransferDialogContactListSearch.value != '')
				{
					this.popupTransferDialogContactListSearch.value = '';
					BX.adjust(this.popupTransferDialogContactListElements, {children: this.messenger.contactListPrepare({ 'viewOffline': false, 'viewChat': false, 'viewGroup': true, 'extra': false, 'searchText': '', 'groupOpen': true})});
				}
		...
		this.popupTransferDialogContactListElements.innerHTML = '';
		BX.adjust(this.popupTransferDialogContactListElements, {children: this.messenger.contactListPrepare({'groupOpen': true, 'viewOffline': false, 'viewGroup': true, 'viewChat': false, 'extra': false, 'searchText': this.popupTransferDialogContactListSearch.value})});
	}, this));
	...
	BX.bindDelegate(this.popupTransferDialogContactListElements, "click", {className: 'bx-messenger-cl-item'}, BX.delegate(function(e) {
		...
		{
			this.popupTransferDialogContactListSearch.value = '';
			BX.adjust(this.popupTransferDialogContactListElements, {children: this.messenger.contactListPrepare({'viewOffline': false, 'viewGroup': true, 'viewChat': false, 'groupOpen': true,  'extra': false, 'searchText': ''})});
		}
		if (this.phoneTransferUser > 0)
		{
			params.maxUsers = params.maxUsers+1;
