...
<style>
	...
	.IPOLSDEK_PC_checkB{
		width: 50px;
		text-align: center !important;
		<?if(!sdekExport::$isLoaded || count($arGoods) < 10){?>
		visibility:hidden;
		<?}?>
	}
	.IPOLSDEK_PC_name{
	...
	var IPOLSDEK_packs = {
		...
		addItem: function(pack,item){
			...
			packHTML += "<div id='IPOLSDEK_pack_"+pack+"_good_"+item+"' class='IPOLSDEK_PC_item'>";
				...
				packHTML += "<div class='IPOLSDEK_PC_cnt'><span class='IPOLSDEK_PC_curCnt'><a href='javascript:void(0)' onclick='IPOLSDEK_packs.renderCnt($(this))'>"+ IPOLSDEK_packs.packs[pack][item] + "</a></span><span class='IPOLSDEK_PC_cChange'><span class='IPOLSDEK_PC_cChange_left'></span>&nbsp;&gt;&nbsp;<input type='text' onchange='IPOLSDEK_packs.onChangeCnt($(this))'></span><span class='IPOLSDEK_PC_unity'>&nbsp;</span></div>";
				packHTML += "<p></p>";
			packHTML += "</div>";
		...
		addPack: function(id){
			...
			<?if(!sdekExport::$isLoaded){?>IPOLSDEK_packs.initUi();<?}?>
		...
		deletePack: function(id){
			...
			for(var i = id;;i++){
				if(typeof(IPOLSDEK_packs.packs[i+1]) == 'undefined'){
					...
					IPOLSDEK_packs.packs[i] = IPOLSDEK_packs.copyObj(IPOLSDEK_packs.packs[i+1]);
					if(typeof(IPOLSDEK_packs.packsGabs[i+1]) != 'undefined'){
						IPOLSDEK_packs.packsGabs[i] = IPOLSDEK_packs.copyObj(IPOLSDEK_packs.packsGabs[i+1]);
		...
		initUi: function(){
			$('.IPOLSDEK_PC_goodsPlace').sortable({connectWith:".IPOLSDEK_PC_goodsPlace",stop:IPOLSDEK_packs.moveItem,start:IPOLSDEK_packs.closeRender}).disableSelection();
		},
		...
		cb_showWindow: function(){
			...
			IPOLSDEK_packs.showWindow("80px",'540px');
		},
		...
		renderCnt: function(link){
			...
			tmpHtml += "</select><p><span id='IPOLSDEK_PC_CChange_cntLeft' maxCnt='"+(extCnt-1)+"'>"+extCnt+"</span>&nbsp;/&nbsp;<input type='text' value='1' id='IPOLSDEK_PC_CChange_cntNew' onchange='IPOLSDEK_packs.onChangeCnt($(this))'></p><p><a href='javascript:void(0)' onclick='IPOLSDEK_packs.confirmChangings()'>OK</a><input id='IPOLSDEK_PC_goodDescr' type='hidden' value='"+(link.closest('.IPOLSDEK_PC_item').attr('id'))+"'></p>";

			$('#IPOLSDEK_PC_separator .pop-text').html(tmpHtml);

			IPOLSDEK_packs.showWindow(($(link).position().top)+'px','540px');
		},
		...
		gabs_requestGabs: function(id){
			...
			$.post("/bitrix/js/<?=self::$MODULE_ID?>/ajax.php",{action:'countGoods',goods: IPOLSDEK_packs.packs[id]},function(data){
		...
		// расчет доставки
		cntDost: function(){
			var packs = IPOLSDEK_packs.getSO();
			delete packs.cnt;
		...
		autoDistribution: function(){
			...
			if(i < 2){
				IPOLSDEK_packs.packs = IPOLSDEK_packs.copyObj(IPOLSDEK_packs.packsSrc);
		...
		open: function(){
			if(!IPOLSDEK_packs.wnd){
				...
				IPOLSDEK_packs.wnd = new BX.CDialog({
					...
					width: '685',
					buttons: [
						'<input type=\"button\" id=\"IPOLSDEK_packApply\" value=\"<?=GetMessage('MAIN_APPLY')?>\" onclick=\"IPOLSDEK_packs.apply()\"/>',
						'<input type=\"button\" id=\"IPOLSDEK_packCancel\" value=\"<?=GetMessage('MAIN_RESET')?>\"  onclick=\"IPOLSDEK_packs.cancel()\"/>',
					]
				});
				IPOLSDEK_packs.load();
			}
			IPOLSDEK_packs.loadPacks();
			IPOLSDEK_packs.showGoods();
			IPOLSDEK_packs.wnd.Show();
		},

		apply: function(){
			var SO = IPOLSDEK_packs.getSO();
			IPOLSDEK_packs.saveObj = SO;

			IPOLSDEK_packs.close();
		...
		copyObj: function(obj){
			...
			var temp = {};
			for(var key in obj)
				temp[key] = IPOLSDEK_packs.copyObj(obj[key]);
			return temp;
		} 
	};
</script>
<div id='IPOLSDEK_PackController'>
	...
	<div id='IPOLSDEK_buttonPlace'>
		<input type='button' onclick='IPOLSDEK_packs.addPack();' value='<?=GetMessage("IPOLSDEK_JS_SOD_addPack")?>'/>&nbsp;
		<span id='IPOLSDEK_PC_cntDost'><input type='button' onclick='IPOLSDEK_packs.cntDost();' value='<?=GetMessage("IPOLSDEK_JS_SOD_cntDost")?>'/>&nbsp;</span>
		<span id='IPOLSDEK_PC_autoCnt'><input type='button' onclick='IPOLSDEK_packs.autoDistribution();' value='<?=GetMessage("IPOLSDEK_JS_SOD_autoDistribution")?>'/>&nbsp;</span>
		<span id='IPOLSDEK_PC_hideGoods' style='display:none;'><input type='button' onclick='IPOLSDEK_packs.hideGoods();' value='<?=GetMessage("IPOLSDEK_JS_SOD_hideGoods")?>'/>&nbsp;</span>
		<span id='IPOLSDEK_PC_showGoods' style='display:none;'><input type='button' onclick='IPOLSDEK_packs.showGoods();' value='<?=GetMessage("IPOLSDEK_JS_SOD_showGoods")?>'/>&nbsp;</span>
	</div>
