...
if (empty($arRunErrors))
{
	...
	{
		$arSelect = array("ID", "LID", "IBLOCK_ID", "IBLOCK_SECTION_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "DETAIL_TEXT", "DETAIL_TEXT_TYPE", "PREVIEW_TEXT_TYPE", "DETAIL_PICTURE", "LANG_DIR", "DETAIL_PAGE_URL");
...
/*					$dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
			...
			foreach ($arYandexFields as $key)
			{
				...
				case 'name':
					...
					$strTmpOff .= "<name>".yandex_text2xml(htmlspecialcharsbx($arAcc["NAME"]))."</name>\n";
					break;
				case 'description':
					$strTmpOff .=
						"<description>".
	...
	{
		$arOfferSelect = array("ID", "LID", "IBLOCK_ID", "NAME", "PREVIEW_PICTURE", "PREVIEW_TEXT", "PREVIEW_TEXT_TYPE", "DETAIL_TEXT", "DETAIL_TEXT_TYPE",  "DETAIL_PICTURE", "DETAIL_PAGE_URL");
				...
				if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
				{
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							$strOfferYandex .= "<name>".yandex_text2xml(htmlspecialcharsbx($arOfferItem["NAME"]))."</name>\n";
							break;
						case 'description':
...
/*							$dbVAT = CCatalogProduct::GetVATInfo($arOfferItem['ID']);
					...
					foreach ($arYandexFields as $key)
					{
						...
						case 'name':
							...
							$strOfferYandex .= "<name>".htmlspecialcharsbx(yandex_text2xml($arOfferItem["NAME"]))."</name>\n";
							break;
						case 'description':
							$strOfferYandex .= "<description>";
							if (strlen($arOfferItem['~DETAIL_TEXT']) <= 0)
...
/*						$dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
				...
				foreach ($arYandexFields as $key)
				{
					...
					case 'name':
						...
						$strValue = "<name>".htmlspecialcharsbx(yandex_text2xml($arItem["NAME"]))."</name>\n";
						break;
					case 'description':
						$strValue =
							"<description>".
							yandex_text2xml(TruncateText(
								($arItem["DETAIL_TYPE"]=="html"?
								strip_tags(preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~DETAIL_TEXT"])) : preg_replace_callback("'&[^;]*;'", "yandex_replace_special", $arItem["~DETAIL_TEXT"])),
								2950), true).
							"</description>\n";
						break;
